...
from trainers import FinetuneTrainer, DistSAModelTrainer, DegenSASRecTrainer, FMLPRecTrainer
from models import S3RecModel
from seqmodels import SASRecModel, DistSAModel, DistMeanSAModel, FMLPRecModel
...
def main():
    ...
    parser.add_argument("--degen_loss", type=str, default="cos", help='degen_metric')
    parser.add_argument("--degen_weight", type=float, default=0.01, help='degen loss weight')
    parser.add_argument("--seq_degen_weight", type=float, default=0.01, help='sequence emb degen loss weight')
    parser.add_argument("--u_weight", type=float, default=0.01, help='degen loss weight')
    parser.add_argument("--v_weight", type=float, default=0.01, help='degen loss weight')
    parser.add_argument("--s_weight", type=float, default=0.01, help='degen loss weight')
    ...
    # save model args
    args_str = f'{args.model_name}-{args.data_name}-{args.hidden_size}-{args.num_hidden_layers}-{args.num_attention_heads}-{args.hidden_act}-{args.attention_probs_dropout_prob}-{args.hidden_dropout_prob}-{args.max_seq_length}-{args.lr}-{args.weight_decay}-{args.ckp}-{args.kernel_param}-{args.pvn_weight}-{args.degen_loss}-{args.degen_weight}-{args.u_weight}-{args.v_weight}-{args.s_weight}-{args.seq_degen_weight}'
    ...
    if args.model_name == 'DistSAModel':
        ...
        trainer = DistSAModelTrainer(model, train_dataloader, eval_dataloader,
                                    test_dataloader, args)
    elif args.model_name == 'SASRecDegenModel':
        model = SASRecModel(args=args)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)
        test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.batch_size)
        trainer = DegenSASRecTrainer(model, train_dataloader, eval_dataloader,
                                test_dataloader, args)
    elif args.model_name == 'FMLPRecModel':
