...
from utils import recall_at_k, ndcg_k, get_metric, cal_mrr, cal_diversity, get_user_performance_perpopularity, get_item_performance_perpopularity
...
class Trainer:
    ...
    def eval_visualization(self, user_seq, args):
        return self.qualitative_analysis(self.test_dataloader, user_seq, args)

    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        raise NotImplementedError

    def eval_analysis(self, dataloader, seqs, args):
        raise NotImplementedError
...
class FinetuneTrainer(Trainer):
    ...
    def eval_analysis(self, dataloader, user_seq, args):
        ...
        with torch.no_grad():
            ...
            _, item_s, _ = torch.svd(self.model.item_embeddings.weight)
            print('Item singular values trend: ', item_s)
            return scores, result_info, None
...
class DegenSASRecTrainer(Trainer):

    def __init__(self, model,
                 train_dataloader,
                 eval_dataloader,
                 test_dataloader, args):
        super(DegenSASRecTrainer, self).__init__(
            model,
            train_dataloader,
            eval_dataloader,
            test_dataloader, args
    ...
    def eval_analysis(self, dataloader, user_seq, args):
        ...
        with torch.no_grad():
            ...
            #for batch in rec_data_iter:
                ...
                recommend_output = recommend_output[:, -1, :]
                user_seq_out_embs.append(recommend_output)
            ...
            get_item_performance_perpopularity(items_in_freqintervals, all_pos_items_ranks, Ks, freq_quantiles, args.item_size)
            _, item_s, _ = torch.svd(self.model.item_embeddings.weight)
            item_s = item_s.cpu().data.numpy()
            item_s = item_s/np.max(item_s)
            print('Item singular values trend: ', np.sum(item_s), item_s)
            seq_embs = torch.cat(user_seq_out_embs, dim=0)
            print(seq_embs.shape)
            _, seq_s, _ = torch.svd(seq_embs)
            seq_s = seq_s.cpu().data.numpy()
            seq_s = seq_s/np.max(seq_s)
            print('Seq singular values trend: ', np.sum(seq_s), seq_s)

            item_embs = self.model.item_embeddings.weight.cpu().data.numpy().copy()
            diversity = cal_diversity(pred_list, item_embs)
            print('diversity: ', diversity)
            return scores, result_info, None

    def degen_seqemb_loss(self, seq_out):
        seq_emb = seq_out.view(-1, self.args.hidden_size)
        B, _ = seq_emb.shape
        if self.args.degen_loss == 'cos':
            cos_sim = torch.mm(seq_emb, seq_emb.transpose(0,1))
            cos_sim.fill_diagonal_(0.0)
            d_loss = 1/(B*B) * torch.sum(cos_sim)
        elif self.args.degen_loss == 'l2':
            d_loss = -1/(B*B) * torch.sum(torch.cdist(seq_emb, seq_emb, p=2))
        elif self.args.degen_loss == 'svd_trend':
            d_loss = -torch.norm(seq_emb, p='nuc')/torch.sum(torch.norm(seq_emb, dim=1))

        return d_loss

    def degen_loss(self):
        item_emb = self.model.item_embeddings.weight
        num_items, _ = item_emb.shape

        item_norm = item_emb / item_emb.norm(dim=1)[:, None]

        d_loss = 0.0

        if self.args.degen_loss == 'cos':
            #d_loss = 1/(num_items*num_items) * torch.sum(cos(item_emb, item_emb))
            cos_sim = torch.mm(item_emb, item_emb.transpose(0,1))
            cos_sim.fill_diagonal_(0.0)
            d_loss = 1/(num_items*num_items) * torch.sum(cos_sim)
        elif self.args.degen_loss == 'l2':
            #cos_sim = torch.mm(item_emb, item_emb.transpose(0,1))
            #cos_sim.fill_diagonal_(0.0)
            #d_loss = 1/num_items* torch.sum(torch.norm(item_emb, dim=1)) + 1/(num_items*num_items) * torch.sum(cos_sim) #
            d_loss = -1/(num_items*num_items) * torch.sum(torch.cdist(item_emb, item_emb, p=2))
        elif self.args.degen_loss == 'svd_trend':
            #l2_loss = -1/(num_items*num_items) * torch.sum(torch.cdist(item_emb, item_emb, p=2))
            #l2_loss = torch.sum(torch.norm(item_emb, dim=1))
            #u_loss = -1/(num_items*num_items) * torch.sum(torch.cdist(u, u, p=2))
            #v_loss = -1/(num_items*num_items) * torch.sum(torch.cdist(v.t(), v.t(), p=2))
            #print(torch.sum(s), torch.norm(item_emb, p='nuc'), l2_loss)
            #s = s/torch.max(s)
            #s_loss = -torch.sum(s)
            #d_loss = self.args.u_weight * u_loss + self.args.v_weight * v_loss + self.args.s_weight * s_loss
            #print(self.args.u_weight * u_loss, self.args.v_weight * v_loss, self.args.s_weight * s_loss)
            #d_loss = self.args.degen_weight *l2_loss + self.args.s_weight * s_loss
            #d_loss = self.args.degen_weight * (s_loss/l2_loss)
            d_loss = -torch.norm(item_emb, p='nuc')/torch.sum(torch.norm(item_emb, dim=1))

        
        #return self.args.degen_weight * d_loss
        return d_loss
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            self.model.train()
            d_weight = self.args.degen_weight
            seq_d_weight = self.args.seq_degen_weight
            rec_avg_loss = 0.0
            rec_cur_loss = 0.0
            rec_avg_auc = 0.0
            rec_d_loss = 0.0
            ...
            for batch in rec_data_iter:
                ...
                sequence_output, _ = self.model.finetune(input_ids)
                loss, batch_auc = self.cross_entropy(sequence_output, target_pos, target_neg)
                #all_d_loss = self.degen_loss()

                seq_d_loss = self.degen_seqemb_loss(sequence_output)
                loss += seq_d_weight * seq_d_loss
                ...
                rec_cur_loss = loss.item()
                rec_avg_auc += batch_auc.item()
                rec_seq_d_loss += seq_d_weight * seq_d_loss.item()

            all_d_loss = d_weight * self.degen_loss()
            self.optim.zero_grad()
            all_d_loss.backward()
            self.optim.step()
            #d_weight *= 0.999
            rec_d_loss += all_d_loss.item()
            ...
            post_fix = {
                ...
                "rec_cur_loss": '{:.4f}'.format(rec_cur_loss),
                "rec_avg_auc": '{:.4f}'.format(rec_avg_auc / len(rec_data_iter)),
                "rec_d_loss": '{:.4f}'.format(rec_d_loss/len(rec_data_iter)),
                "rec_seq_d_loss": '{:.4f}'.format(rec_seq_d_loss/len(rec_data_iter)),
            }
    ...
    def qualitative_analysis(self, dataloader, user_seq, args):
        ...
        with torch.no_grad():
            ...
            #for batch in rec_data_iter:
                ...
                batch = tuple(t.to(self.device) for t in batch)
                user_ids, input_ids, target_pos, target_neg, answers = batch
                recommend_mean_output, recommend_cov_output, att_scores, _ = self.model.finetune(input_ids, user_ids)
