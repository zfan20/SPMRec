...
def cal_diversity(predicted, embeddings):
    avg_diversity = 0.0
    num_users = len(predicted)
    dim = embeddings.shape[1]
    for i in range(num_users):
        pred_list = predicted[i][:10]
        pred_embs = np.zeros((10, dim))
        for ind, each_pred_i in enumerate(pred_list):
            pred_embs[ind, :] = embeddings[each_pred_i, :]
        #pred_sim = pairwise_distances(pred_embs, metric='cosine')
        pred_sim = cosine_similarity(pred_embs)
        user_diversity = 1 - (pred_sim.sum() - np.diag(pred_sim).sum())/2
        avg_diversity += user_diversity

    return avg_diversity / num_users
